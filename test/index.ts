...
describe("Lottery", function () {
  ...
  before(async () => {
    ...
    ticketPrice = (await lottery.ticketPrice()).toNumber();
  });
  ...
  it("Should buy one ticket", async function () {
    ...
    expect(await lottery.totalTickets()).to.equal(1);
  ...
  it("Should buy three tickets", async function () {
    ...
    expect(await lottery.ticketsToPlayers(0)).to.equal(owner.address);
    expect(await lottery.ticketsToPlayers(1)).to.equal(owner.address);
    expect(await lottery.ticketsToPlayers(2)).to.equal(owner.address);
  })
  ...
  it("Should buy three tickets with one accounts and three with another", async function () {
    ...
    expect(await lottery.totalTickets()).to.equal(6);
    expect(await mok.balanceOf(lottery.address)).to.equal(ticketPrice * 6);
    expect(await lottery.ticketsToPlayers(3)).to.equal(addr1.address);
    expect(await lottery.ticketsToPlayers(4)).to.equal(addr1.address);
    expect(await lottery.ticketsToPlayers(5)).to.equal(addr1.address);
  })
  ...
  it("Addr1 should buy the lottery and owner should draw the lottery and addr1 should win the lottery", async function () {
    ...
    expect(await mok.balanceOf(lottery.address)).to.equal(await lottery.usageFees());
  })
